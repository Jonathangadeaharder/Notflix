...
const ENV = {
  ...process.env,
  DATABASE_URL: "postgres://admin:password@127.0.0.1:5432/main_db",
...
function log(msg) {
  console.log(`\x1b[36m[Manager]\x1b[0m ${msg}`);
}
function error(msg) {
...
function getDockerCmd() {
  if (IS_WSL || IS_WIN) {
    try {
      execSync("docker.exe --version", { stdio: "ignore" });
      return "docker.exe";
    } catch (e) {
      return "docker";
    }
...
async function main() {
  ...
  try {
    const isRunning =
      execSync(`${docker} ps --filter "name=notflix-db" --format "{{.Names}}"`)
        .toString()
        .trim() === "notflix-db";
    if (!isRunning) {
      const exists =
        execSync(
          `${docker} ps -a --filter "name=notflix-db" --format "{{.Names}}"`,
        )
          .toString()
          .trim() === "notflix-db";
      if (exists) {
        ...
        execSync(
          `${docker} run -d --name notflix-db -p 5432:5432 -e POSTGRES_USER=admin -e POSTGRES_PASSWORD=password -e POSTGRES_DB=main_db -v notflix-pg-data:/var/lib/postgresql/data postgres:16-alpine`,
        );
      }
    }
  } catch (e) {
