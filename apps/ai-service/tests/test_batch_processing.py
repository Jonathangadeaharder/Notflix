...
def mock_transformers(monkeypatch):
    ...
    mock_tokenizer.batch_decode.side_effect = (
        lambda x, skip_special_tokens: [f"trans_{i}" for i in range(len(x))]
    )
...
def test_filter_analyze_batch_real():
    """
    Verifies that analyze_batch returns a list of lists of TokenAnalysis
    using Real Spacy (en_core_web_sm).
    """
...
def test_translate_batch():
    ...
    # We need to monkeypatch the models dict directly
    # or the class logic will try to load real models
    # But since we mocked from_pretrained, it should be fine.

    texts = ["hola", "mundo"]

    # We need to override the batch_decode behavior to match
    # our input length for this test to be meaningful
    ...
    with pytest.MonkeyPatch.context() as m:
        ...
        mock_tokenizer.batch_decode.side_effect = (
            lambda gens, skip_special_tokens: ["trans_hola", "trans_mundo"]
        )
