...
test.describe("Transcription Service", () => {
  ...
  test("POST /transcribe should return segments using tiny model", async ({
    request,
  }) => {
    // 1. Prepare Test File
    // Pass strictly relative filename; the docker AI service internally resolves this via MEDIA_ROOT to /app/media/test_audio.mp3
    const apiPayloadPath = "test_audio.mp3";
