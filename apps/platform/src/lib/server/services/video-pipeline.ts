import { globalEvents, EVENTS, type VideoUploadedPayload, type TranscriptionCompletedPayload, type AnalysisCompletedPayload } from '../infrastructure/event-bus';
import { mapAnalysisToSegments, extractUniqueLemmas, extractUnknownLemmas, mapTranslationsToSegments } from '../domain/translation-core';
...
export function registerPipelineListeners(db: typeof drizzleDb, aiGateway: IAiGateway, filter: SmartFilter) {
    
    // 1. UPLOAD -> TRANSCRIBE
    globalEvents.on(EVENTS.VIDEO_UPLOADED, async (payload: VideoUploadedPayload) => {
        console.log(`[Pipeline] Video Uploaded: ${payload.videoId}. Initiating pipeline.`);
        ...
        try {
            ...
            globalEvents.emit(EVENTS.PROCESSING_UPDATE, { videoId: payload.videoId, status: 'TRANSCRIBING', percent: 10 });

            const [record] = await db.select().from(video).where(eq(video.id, payload.videoId)).limit(1);
            if (!record) throw new Error("Video not found");

            const aiPath = toAiServicePath(record.filePath);
            const transcription = await aiGateway.transcribe(aiPath, payload.targetLang);
            ...
            if (!record.filePath.match(/\.(mp3|wav|m4a|aac|ogg)$/i)) {
                aiGateway.generateThumbnail(aiPath).then(async (res) => {
                    await db.update(video).set({ thumbnailPath: res.thumbnail_path }).where(eq(video.id, payload.videoId));
                }).catch(e => console.warn("Thumbnail failed", e));
            }

        } catch (err) {
            console.error(`[Pipeline Error] TRANSCRIBE:`, err);
            await db.update(videoProcessing).set({ status: ProcessingStatus.ERROR }).where(eq(videoProcessing.videoId, payload.videoId));
        }
    });

    // 2. TRANSCRIBE -> ANALYZE
    globalEvents.on(EVENTS.TRANSCRIPTION_COMPLETED, async (payload: TranscriptionCompletedPayload) => {
        console.log(`[Pipeline] Transcription complete for: ${payload.videoId}. Starting Analysis.`);
        try {
    ...
    globalEvents.on(EVENTS.ANALYSIS_COMPLETED, async (payload: AnalysisCompletedPayload) => {
        console.log(`[Pipeline] Analysis complete for: ${payload.videoId}. Starting Translation Enrichment.`);
        try {
            ...
            if (!payload.userId) {
                ...
                const [lemmaRes, sentenceRes] = await Promise.all([
                    lemmaList.length > 0 ? aiGateway.translate(lemmaList, payload.targetLang, payload.nativeLang) : { translations: [] },
                    aiGateway.translate(sentenceTexts, payload.targetLang, payload.nativeLang)
                ]);
            ...
            globalEvents.emit(EVENTS.PROCESSING_UPDATE, { videoId: payload.videoId, status: ProcessingStatus.COMPLETED, percent: 100 });
            console.log(`[Pipeline] Processing fully complete for: ${payload.videoId}.`);
