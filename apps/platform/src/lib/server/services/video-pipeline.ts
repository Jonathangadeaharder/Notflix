  ...
  try {
    ...
    const transcription = await aiGateway.transcribeWithProgress(
      ...
      async (percent) => {
        ...
        const now = Date.now();
        const advanced = clamped > lastPersistedPercent;
        const intervalElapsed = now - lastPersistedAt >= PROGRESS_DB_MIN_INTERVAL_MS;
        if (!advanced && !intervalElapsed) return;
    ...
    // Thumbnail (fire-and-forget, non-blocking)
    ...
    console.log(`[Pipeline] Transcription complete for: ${videoId}. Starting Analysis.`);
    ...
    let finalSegments = mapAnalysisToSegments(transcription, batchAnalysis.results);

    // 4. TRANSLATE with linguistic filter (auth required — no guest branch)
    await setStage(db, videoId, targetLang, "TRANSLATING", 93);
    console.log(`[Pipeline] Analysis complete for: ${videoId}. Starting Translation.`);
    ...
    console.error(
      `[Pipeline Error] videoId=${videoId}: ${message}`,
      stack ?? "",
    );
    await db
      .update(videoProcessing)
