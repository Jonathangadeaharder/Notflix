...
const defaultDependencies: DeleteVideoDependencies = {
	getVideoById: async (videoId) => {
		const [record] = await defaultDb.select().from(video).where(eq(video.id, videoId)).limit(1);
		if (!record) {
			return null;
		}
		return {
	...
	deleteVideoProcessingById: (videoId) =>
		defaultDb.delete(videoProcessing).where(eq(videoProcessing.videoId, videoId)),
	deleteVideoById: (videoId) => defaultDb.delete(video).where(eq(video.id, videoId)),
...
export async function deleteVideoAndAssets(
	...
	if (videoRecord.filePath) {
		deletionPromises.push(safelyDeleteFile(videoRecord.filePath, dependencies.deleteFile));
	}
	if (videoRecord.thumbnailPath) {
		deletionPromises.push(safelyDeleteFile(videoRecord.thumbnailPath, dependencies.deleteFile));
	}
...
async function safelyDeleteFile(path: string, deleteFile: (path: string) => Promise<void>): Promise<void> {
	try {
