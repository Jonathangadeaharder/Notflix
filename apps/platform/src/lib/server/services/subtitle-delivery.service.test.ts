...
import { SubtitleDeliveryError, buildSubtitleResponse } from './subtitle-delivery.service';
...
describe('buildSubtitleResponse', () => {
	it('returns vtt response for valid mode', async () => {
		...
		const response = await buildSubtitleResponse('video-1', 'native', service as never);
	...
	it('throws 400 for invalid subtitle mode', async () => {
		await expect(
			buildSubtitleResponse('video-1', 'invalid', { generateVtt: vi.fn() } as never)
	...
	it('throws 404 when subtitles are missing', async () => {
		await expect(
			buildSubtitleResponse(
				'video-1',
				'native',
				{ generateVtt: vi.fn().mockResolvedValue(null) } as never
			)
