...
export async function buildSubtitleResponse(
	...
	if (!videoId) {
		throw new SubtitleDeliveryError(HTTP_STATUS.BAD_REQUEST, 'Video ID is required');
	}

	const mode = normalizeMode(modeParam);
	const vttContent = await service.generateVtt(videoId, mode);
	if (!vttContent) {
		throw new SubtitleDeliveryError(HTTP_STATUS.NOT_FOUND, 'Subtitles not found');
	}
...
function normalizeMode(mode: string | null): SubtitleMode {
	...
	throw new SubtitleDeliveryError(HTTP_STATUS.BAD_REQUEST, 'Invalid subtitle mode');
}
