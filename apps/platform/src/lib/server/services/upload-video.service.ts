...
export async function handleVideoUpload(
	...
	const filePath = buildUploadFilePath(dependencies.uploadDir, videoId, validation.data.file.name);
...
export async function saveUploadedFileToDisk(file: File, filePath: string): Promise<void> {
...
function validateUploadPayload(
	payload: UploadPayload
): { ok: true; data: { title: string; targetLang: string; file: File } } | { ok: false; value: UploadFailure } {
	...
	if (!parsed.success || !payload.file || payload.file.size === 0) {
		...
		const fileErrors = !payload.file || payload.file.size === 0 ? ['File is required'] : undefined;
...
function buildUploadFilePath(uploadDir: string, videoId: string, originalFileName: string): string {
