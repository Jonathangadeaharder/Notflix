<script lang="ts">
  ...
  const POLLING_INTERVAL_MS = TIME.POLLING_INTERVAL_MS;
  ...
  function getProgressLabel(video: {
    status: string | null;
    progressStage?: string | null;
    progressPercent?: number | null;
  }) {
    if (video.status === "COMPLETED") return "Ready";
    if (video.status === "ERROR") return "Processing failed";
    return `${video.progressStage || "QUEUED"} • ${video.progressPercent || 0}%`;
  }
  ...
  function getProgressBarPercent(video: {
    status: string | null;
    progressPercent?: number | null;
  }) {
  ...
  >
    {#each data.videos as video (video.id)}
      ...
      >
        ...
        <Card.Content class="p-5 pt-4 mt-2 border-t border-white/5 space-y-4">
          <div class="space-y-2">
            <div
              class="flex items-center justify-between text-[11px] font-semibold uppercase tracking-widest text-zinc-500"
            >
              <span>{getProgressLabel(video)}</span>
              <span>{video.progressPercent || 0}%</span>
            </div>
            <div class="h-2 rounded-full bg-white/5 overflow-hidden">
              <div
                class="h-full rounded-full {getProgressBarClass(video.status)}"
                style={`width: ${getProgressBarPercent(video)}%`}
              ></div>
            </div>
          </div>
