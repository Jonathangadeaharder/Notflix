<script lang="ts">
  ...
  const FILE_INPUT_CLASSES =
    "block w-full text-sm text-zinc-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-zinc-800 file:text-zinc-300 hover:file:bg-zinc-700";
  const TEXT_INPUT_CLASSES =
    "bg-black/50 border border-zinc-700 text-white p-2 rounded w-full max-w-xs";
  const ACTION_BUTTON_CLASSES =
    "bg-red-600 text-white px-6 py-2 rounded-full font-bold hover:bg-red-700 disabled:opacity-50 transition-colors";
  ...
  async function handlePipeline() {
    ...
    try {
      ...
      const transcriptionBlob = await runProcess(
        "/api/debug/transcribe",
        {
          file: pipelineFile[0],
      ...
      const srtFile = new File([transcriptionBlob], "temp.srt", {
        type: "text/plain",
      });

      const analysisBlob = await runProcess(
        "/api/debug/filter",
        {
          file: srtFile,
          language: pipelineSourceLang,
      ...
      const analysisFile = new File([analysisBlob], "temp_analyzed.srt", {
        type: "text/plain",
      });
...
<div class="container mx-auto p-8">
  ...
  <div class="flex mb-4 border-b border-zinc-800">
    <button
      class="px-4 py-2 mr-2 {activeTab === 'transcribe'
    ...
    <button
      class="px-4 py-2 mr-2 {activeTab === 'filter'
        ? 'border-b-2 border-red-500 font-bold text-white'
        : 'text-zinc-400'}"
    ...
    <button
      class="px-4 py-2 mr-2 {activeTab === 'translate'
        ? 'border-b-2 border-red-500 font-bold text-white'
        : 'text-zinc-400'}"
    ...
    <button
      class="px-4 py-2 {activeTab === 'pipeline'
        ? 'border-b-2 border-red-500 font-bold text-white'
        : 'text-zinc-400'}"
  ...
  <div class="p-6 border border-zinc-800 rounded-xl shadow-md bg-zinc-900">
    {#if activeTab === "transcribe"}
      ...
      <div class="mb-4">
        <label class="block">
          ...
          <input
            type="file"
            accept="video/*,audio/*"
            onchange={(e) => (transcribeFile = e.currentTarget.files!)}
      ...
      {#if videoPreviewUrl}
        <div class="mb-4">
          ...
          <video
            src={videoPreviewUrl}
            controls
            class="w-full max-h-96 bg-black rounded border border-zinc-800"
          ></video>
        </div>
      {/if}
      ...
      <div class="mb-4">
        <label class="block">
          <span class="block mb-2 font-medium text-zinc-300"
            >Language (e.g. es, en)</span
          >
          <input
            type="text"
            bind:value={transcribeLang}
      ...
      <div class="mb-4">
        <label class="block">
          ...
          <input
            type="file"
            accept=".srt"
            onchange={(e) => (filterFile = e.currentTarget.files!)}
            class={FILE_INPUT_CLASSES}
          />
        </label>
      </div>
      ...
      <div class="mb-4">
        <label class="block">
          ...
          <input
            type="file"
            accept=".srt"
            onchange={(e) => (translateFile = e.currentTarget.files!)}
            class={FILE_INPUT_CLASSES}
          />
        </label>
      </div>
        ...
        <div>
          <label class="block">
            ...
            <input
              ...
              class="bg-black/50 border border-zinc-700 text-white p-2 rounded w-full"
            />
          </label>
        </div>
        <div>
    ...
    {:else if activeTab === "pipeline"}
      <h2 class="text-xl font-semibold mb-4 text-white">
        Full Pipeline (Transcribe -> Analyze -> Translate)
      </h2>
      <div class="mb-4">
        <label class="block">
          <span class="block mb-2 font-medium text-zinc-300">Video File</span>
          <input
            type="file"
            accept="video/*,audio/*"
            onchange={(e) => (pipelineFile = e.currentTarget.files!)}
            class={FILE_INPUT_CLASSES}
          />
        </label>
      </div>
      ...
      <div class="grid grid-cols-2 gap-4 mb-4 max-w-md">
        <div>
          <label class="block">
            <span class="block mb-2 font-medium text-zinc-300"
              >Source Language (Video)</span
            >
            <input
              type="text"
              bind:value={pipelineSourceLang}
              class="bg-black/50 border border-zinc-700 text-white p-2 rounded w-full"
            />
          </label>
        </div>
        <div>
          <label class="block">
            <span class="block mb-2 font-medium text-zinc-300"
              >Target Language (Translation)</span
            >
            <input
              type="text"
              bind:value={pipelineTargetLang}
              class="bg-black/50 border border-zinc-700 text-white p-2 rounded w-full"
            />
          </label>
        </div>
